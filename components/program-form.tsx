...
export function ProgramForm({ program, preview, onboarding = false }: { program: Program; preview?: Omit<BrandPreviewProps, "rewardThreshold" | "rewardLabel" | "unit">; onboarding?: boolean }) {
  ...
  async function submit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMsg(r.ok ? "Programme enregistré." : "Impossible d’enregistrer. Vérifie les valeurs du programme.");
  ...
  </form>{preview && <aside className="card"><h3>Aperçu de ta carte</h3><BrandPreview {...preview} rewardThreshold={Number(threshold) || 0} rewardLabel={reward} unit={mode === "STAMPS" ? "tampons" : "points"}/></aside>}</div>;
}
