...
export default async function DashboardPage() {
  ...
  return <><AppNav restaurantName={restaurant.name}/><main className="shell page">
    ...
    <section className="card" style={{marginTop:18}}><div className="section-head"><div><h3>Dernières transactions</h3><p className="muted">{stats.active_week} clients actifs sur 7 jours</p></div><Link className="btn" href="/dashboard/transactions">Tout voir</Link></div><div className="table-wrap"><table><thead><tr><th>Client</th><th>Action</th><th>Variation</th><th>Solde</th><th>Date</th></tr></thead><tbody>{latest.map((r:any)=><tr key={r.id}><td>{r.first_name || r.short_code}</td><td>{r.type}</td><td>{Number(r.delta)>0?"+":""}{r.delta}</td><td>{r.balance_after}</td><td>{new Date(r.created_at).toLocaleString("fr-FR")}</td></tr>)}</tbody></table></div></section>
    <section className="grid grid-2" style={{marginTop:18}}><div className="card"><h3>QR d’inscription</h3><p className="muted">Lien public du commerce : <code>/j/{restaurant.slug}</code></p><Link className="btn" href="/dashboard/poster">Créer l’affiche A4</Link></div><div className="card"><h3>Wallet</h3><p className="muted">Le schéma Apple/Google est prévu dans la base. L’intégration réelle reste derrière un feature flag tant que le scan n’a pas passé le test terrain.</p></div></section>
  </main></>;
}
