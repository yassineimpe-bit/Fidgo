...
export default async function WalletDashboardPage() {
  ...
  return <><AppNav restaurantName={restaurant?.name}/><main className="shell page">
    ...
    <section className="grid grid-3">
      <div className="card"><h3>HTTPS</h3><StatusPill ok={status.appUrlConfigured && status.appUrlHttps}>{status.appUrlConfigured && status.appUrlHttps ? "Prêt" : "À configurer"}</StatusPill><p className="muted" style={{marginTop:10}}>NEXT_PUBLIC_APP_URL doit pointer vers l’URL HTTPS publique de Fidgo.</p></div>
      <div className="card"><h3>Google Wallet</h3><StatusPill ok={status.google.configured}>{status.google.configured ? "Prêt" : status.google.enabled ? "Incomplet" : "Désactivé"}</StatusPill><p className="muted" style={{marginTop:10}}>{status.google.missing.length ? `Manque : ${status.google.missing.join(", ")}` : "Issuer et service account présents."}</p><p><strong>{countFor("GOOGLE","active")}</strong> pass actif(s)</p></div>
      <div className="card"><h3>Apple Wallet</h3><StatusPill ok={status.apple.configured}>{status.apple.configured ? "Prêt" : status.apple.enabled ? "Incomplet" : "Désactivé"}</StatusPill><p className="muted" style={{marginTop:10}}>{status.apple.missing.length ? `Manque : ${status.apple.missing.join(", ")}` : "Pass Type ID et certificats présents."}</p><p><strong>{countFor("APPLE","active")}</strong> pass actif(s)</p></div>
    </section>

    <section className="card" style={{marginTop:18}}><h3>Chemin vers le premier pass réel</h3><ol style={{lineHeight:1.8,paddingLeft:22}}><li>Base PostgreSQL et HTTPS opérationnels.</li><li>Créer une carte client depuis le QR d’inscription.</li><li>Configurer Google Wallet ou Apple Wallet dans les variables d’environnement.</li><li>Ouvrir la carte client et toucher le bouton Wallet correspondant.</li><li>Créditer la carte au scanner et vérifier que le solde du Wallet se met à jour.</li></ol></section>
