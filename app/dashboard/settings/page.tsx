...
export default async function SettingsPage(){
  ...
  const [restaurant]=await sql`
    select e.name,e.slug,e.logo_url,e.primary_color,to_jsonb(e)->>'secondary_color' as secondary_color,to_jsonb(e)->>'card_background' as card_background,e.address,e.phone,e.instagram,e.website,
  ...
  return <><AppNav restaurantName={restaurant.name}/><main className="shell page"><div className="section-head"><div><h2>Commerce</h2><p className="muted">Identité affichée sur la carte fidélité et les supports d’inscription.</p></div></div><RestaurantForm
    restaurant={restaurant as { name:string; logo_url?:string|null; primary_color:string; secondary_color?:string|null; card_background?:string|null; address?:string|null; phone?:string|null; instagram?:string|null; website?:string|null }}
