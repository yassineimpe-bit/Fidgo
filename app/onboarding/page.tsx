...
export default async function OnboardingPage({ searchParams }: { searchParams: Promise<{ step?: string }> }) {
  ...
  const brand = { name: String(restaurant.name), logoUrl: String(restaurant.logo_url || ""), primaryColor: String(restaurant.primary_color), secondaryColor: restaurant.secondary_color ? String(restaurant.secondary_color) : null, cardBackground: restaurant.card_background ? String(restaurant.card_background) : null, cardImageUrl: restaurant.card_image_id ? cardImagePath(String(restaurant.card_image_id)) : null, qr };
