  ...
  try {
    const result = await sql.begin(async (tx) => {
      const [idem] = await tx`
        ...
        limit 1
      `;
      if (idem) return { balance: Number(idem.balance_after), duplicate: true };
      ...
      const [card] = await tx`
        ...
        where c.token = ${token} and c.establishment_id = ${session.establishmentId}
        for update of c
      `;
      ...
      await tx`
        insert into transactions (
          establishment_id, card_id, staff_user_id, type, delta, balance_after, unit, idempotency_key
        ) values (
          ${session.establishmentId}, ${card.id}, ${session.staffId}, 'redeem', ${-threshold}, ${balance}, ${unit}, ${idempotencyKey}
        )
      `;
      ...
      await tx`
        ...
        values (${session.establishmentId}, ${session.staffId}, 'LOYALTY_REDEEM', 'card', ${String(card.id)}, ${tx.json({ threshold, balance })})
      `;
      return { balance, duplicate: false, rewardLabel: card.reward_label };
    });

    return Response.json({ ...result, serverMs: Date.now() - started });
  } catch (error) {
