...
async function handlePost(req: Request) {
  ...
  if (!isValidEmailVerificationToken(token) || !password || password.length > 256) {
  ...
  try {
    // Le bcrypt est volontairement fait hors transaction pour ne pas garder
    // un verrou PostgreSQL pendant son coût CPU. La transaction revalide
    // ensuite le même hash : une réinscription concurrente rend le lien caduc.
    const [candidate] = await sql`
      select v.staff_user_id, s.password_hash
    ...
    const passwordHash = String(candidate.password_hash);
    const passwordOk = await bcrypt.compare(password, passwordHash);
    if (!passwordOk) {
      return Response.json({ error: "INVALID_CREDENTIALS" }, { status: 401, headers: PRIVATE_HEADERS });
    }
    ...
    const result = await sql.begin(async (tx) => {
      const [row] = await tx`
        ...
        where v.token_hash = ${tokenHash}
          ...
          and v.expires_at > now()
          and s.email_verified_at is null
          and s.password_hash = ${passwordHash}
          and s.active = true
          and e.status = 'active'
        limit 1
        for update of v, s
      ...
      await tx`
        ...
        where id = ${staffId} and email_verified_at is null
      `;
      await tx`
