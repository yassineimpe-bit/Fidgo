...
async function processResend(email: string) {
  try {
    const [staff] = await sql`
      select s.id as staff_id, s.establishment_id, e.name as commerce_name
      from staff_users s
    ...
    const commerceName = String(staff.commerce_name || "votre commerce");
    ...
    // une panne d'envoi ne casse pas un lien précédent encore valable.
