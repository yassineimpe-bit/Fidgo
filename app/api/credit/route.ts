...
export async function POST(req: Request) {
  ...
  if (body.purchaseAmountCents !== undefined && (!Number.isInteger(purchaseAmountCents) || purchaseAmountCents <= 0 || purchaseAmountCents > 10_000_000)) {
    return Response.json({ error: "INVALID_AMOUNT" }, { status: 400 });
  }

  try {
    const result = await sql.begin(async (tx) => {
      const [idem] = await tx`select balance_after, delta from transactions where establishment_id=${session.establishmentId} and idempotency_key=${idempotencyKey} limit 1`;
      if (idem) return { balance: Number(idem.balance_after), delta: Number(idem.delta), duplicate: true };
      ...
      const [card] = await tx`
        ...
        where c.token=${token} and c.establishment_id=${session.establishmentId} and u.deleted_at is null
        for update of c
      `;
      ...
      return { balance, delta, duplicate:false, rewardAvailable: balance >= Number(card.reward_threshold), threshold:Number(card.reward_threshold), rewardLabel:card.reward_label, firstName:card.first_name, mode:card.mode };
    });
    return Response.json({ ...result, serverMs: Date.now() - started }, { headers: { "cache-control": "no-store" } });
  } catch (error) {
