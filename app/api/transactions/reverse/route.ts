  ...
  try {
    const result = await sql.begin(async (tx) => {
      const [existing] = await tx`
        ...
        where establishment_id = ${session.establishmentId} and idempotency_key = ${String(idempotencyKey)}
        limit 1
      `;
      if (existing) return { balance: Number(existing.balance_after), duplicate: true };

      const [original] = await tx`
        select t.id, t.card_id, t.delta, t.unit, t.type, c.balance
        from transactions t
        join cards c on c.id = t.card_id
        where t.id = ${String(transactionId)} and t.establishment_id = ${session.establishmentId}
        for update of c
      `;
      ...
      await tx`
        insert into transactions (
          establishment_id, card_id, staff_user_id, type, delta, balance_after, unit,
          idempotency_key, reversed_transaction_id
        ) values (
          ${session.establishmentId}, ${original.card_id}, ${session.staffId}, 'reversal', ${delta}, ${balance},
          ${original.unit}, ${String(idempotencyKey)}, ${original.id}
        )
      `;
      ...
      await tx`
        ...
        values (${session.establishmentId}, ${session.staffId}, 'TRANSACTION_REVERSE', 'transaction', ${String(original.id)}, ${tx.json({ delta, balance })})
      `;
