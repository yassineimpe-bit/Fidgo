...
function emailVerificationDocument(commerceName: string, verificationUrl: string) {
  const safeCommerce = escapeHtml(cleanSubjectPart(commerceName) || "votre commerce");
...
<html lang="fr" dir="ltr">
  ...
  <body lang="fr" dir="ltr" style="margin:0;background:#f7f6f2;color:#171717;font-family:Arial,sans-serif;">
    <div style="display:none;max-height:0;overflow:hidden;opacity:0;">Confirmez la création de l’espace Retiko de ${safeCommerce}. Ce lien expire dans 24 heures.</div>
    <main style="max-width:600px;margin:0 auto;padding:32px 20px;">
      <div style="background:#ffffff;border:1px solid #e5e3de;border-radius:16px;padding:32px 24px;">
        <p style="margin:0 0 20px;color:#6d6a64;font-weight:700;">RETIKO</p>
        <h1 style="margin:0 0 16px;font-size:26px;line-height:1.25;">Vérifiez votre adresse e-mail</h1>
        <p style="margin:0 0 16px;line-height:1.6;">Une inscription a été lancée pour le commerce <strong>${safeCommerce}</strong>.</p>
        <p style="margin:0 0 24px;line-height:1.6;">Si vous êtes bien à l’origine de cette inscription, ouvrez le lien puis confirmez aussi le mot de passe choisi lors de l’inscription.</p>
        <p style="margin:0 0 24px;">
          <a href="${safeUrl}" style="display:inline-block;background:#ff5a45;color:#ffffff;text-decoration:none;font-weight:700;border-radius:10px;padding:14px 22px;">Vérifier mon adresse e-mail</a>
        </p>
        <p style="margin:0 0 12px;line-height:1.6;">Ce lien expire dans 24 heures et ne fonctionne qu’une seule fois.</p>
        <p style="margin:0;line-height:1.6;color:#6d6a64;"><strong>Vous n’êtes pas à l’origine de cette inscription ?</strong> Ignorez ce message. Ne validez pas le compte.</p>
      </div>
    </main>
  </body>
</html>`;
...
export async function sendEmailVerificationEmail(
  ...
  const commerceName = cleanSubjectPart(input.commerceName) || "votre commerce";
  const payload = {
    from,
    to: [to],
    reply_to: replyTo,
    subject: `Vérifiez l’inscription Retiko de ${commerceName}`,
    text: [
      "Vérifiez votre adresse e-mail",
      "",
      `Une inscription a été lancée pour le commerce ${commerceName}.`,
      "Si vous êtes bien à l’origine de cette inscription, ouvrez le lien puis confirmez aussi le mot de passe choisi lors de l’inscription.",
      "",
      `Vérifier mon adresse e-mail : ${input.verificationUrl}`,
      "",
      "Ce lien expire dans 24 heures et ne fonctionne qu'une seule fois.",
      "Vous n'êtes pas à l'origine de cette inscription ? Ignorez ce message et ne validez pas le compte.",
    ].join("\n"),
    html: emailVerificationDocument(commerceName, input.verificationUrl),
  };
