...
| Opposition / retrait du consentement marketing | Aucune campagne n'est envoyée aujourd'hui. **Outil** : bouton « Retirer le consentement marketing » sur la fiche client (tracé `CUSTOMER_MARKETING_WITHDRAWN`). Le commerce peut seulement retirer : un consentement ne peut être donné que par le client lui-même, à l'inscription | **Libre-service** : case « nouveautés Retiko » dans *Sécurité* (`PATCH /api/account/marketing`), activable et révocable, datée, tracée `STAFF_MARKETING_CONSENT_UPDATED` |
